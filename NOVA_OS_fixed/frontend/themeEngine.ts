...
async function purchaseTheme(name: string): Promise<boolean> {
  const res = await fetch('/api/purchase-theme', {
    ...
    // Include a user_id in the request so that the backend can attribute the
    // purchase.  In a real application this could be replaced with an
    // authenticated user identifier (e.g. from a JWT or session).  For
    // now we default to "anonymous" so that purchases succeed without
